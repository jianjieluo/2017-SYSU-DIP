...
def equalize_hist(input_img):
    ...
    newIm.save('./output/equalize_hist_img29.png')
    # diff(input_img, newIm)
    return newIm
...
def filter2d(input_img, filter_):
    ...
    for x in range(height):
        for y in range(width):
            submatrix = matrix[x+m-1-a:x+m-1+a+1, y+n-1-b:y+n-1+b+1]
...
if __name__ == '__main__': 
    ...
    save_hist(img, "The Original Img's Hist")
    save_hist(equalize_hist(img), "After Equalization")
    
    filter2d(img, np.ones((3,3)) / 9).save('./output/mean_filter3x3.png')
    filter2d(img, np.ones((7,7)) / 49).save('./output/mean_filter7x7.png')
    filter2d(img, np.ones((11,11)) / 121).save('./output/mean_filter11x11.png')
    filter2d(img, np.array([[0,1,0], [1,-4,1], [0,1,0]])).save('./output/lap1.png')
    filter2d(img, np.array([[1,1,1], [1,-8,1], [1,1,1]])).save('./output/lap2.png')
    high_boost(img, 3).save('./output/high_boost3.png')